...
Ahora que contamos con las herramientas necesarias para abordar nuestro problema, procedemos a enunciarlo de manera rigurosa. Este capítulo está completamente basado en \cite[pp. 68-71, 75-86.]{KUZGUN202268}. Consideremos la ecuación estocástica del calor 
...
Del capítulo 1 tenemos garantizada la existencia y unicidad de la solución en el sentido mild, $u(t,x)$, a la ecuación anterior en el caso en que $\sigma$ es una función Lipschitz tal que $\sigma(1)\neq0$, y que $u_0$ es la función idénticamente 1. Con dicha solución al alcance, la cual es de naturaleza estocástica, nos interesamos ahora por en el comportamiento asintótico de un nuevo objeto construido a partir de la misma: los promedios espaciales. Estos objetos, en nuestro contexto, se definen a continuación. 

\begin{dfn} 
 Sea $u=\left\{u(t,x):(t,x)\in [0,\infty)\times\R\right\}$ la solución mild a la ecuación \eqref{shedefinitiva}, con condición inicial $u_0\equiv 1$ y $\sigma:\R\to \R$ una función Lipschitz tal que $\sigma(1)\neq 0$. Para $R>0$ fijo, definimos los promedios espaciales centrados y normalizados de $u$ como 
 \begin{equation*}
...
\end{equation*}
 \end{dfn}
Nos referiremos a las variables $F_{R,t}$ simplemente como los promedios espaciales de la solución $u$, entendiéndolos siempre en el sentido de la definición anterior. Tal y como se mencionó al inicio, en \cite{HUANG20207170}, Huang, Nualart y Viitasaari estudian el comportamiento límite de las variables $F_{R,t}$ conforme $R$ tiende a infinito, siendo capaces de probar resultados tipo teorema central del límite para $F_{R,t}$. Más aún, en dicha publicación, utilizando método de Stein junto con cálculo de Malliavin, se obtuvieron cotas superiores para la tasa de convergencia de $F_{R,t}$ a la distribución normal estándar en distancia de variación total. De manera precisa, obtuvieron el siguiente resultado
\begin{equation}
   d_{\text{TV}}(F_{R,t},N)\leq \frac{C_t}{\sqrt{R}}, \qquad \text{para }t>0 \ \text{ y } \ R\geq1.  
\end{equation}
...
Sea $u=\left\{u(t,x):(t,x)\in [0,\infty)\times\R\right\}$ la solución mild de la ecuación estocástica del calor dada antes, con condición inicial $u_0(x)\equiv 1$. Supongamos que $\sigma:\R\to\R$ es una función Lipschitz tal que $\sigma(1)\neq 0$ y además cumple que
\begin{itemize}
   \item $\sigma\in C^2(\R)$.
...
\]
Entonces para $t>0$ fijo, se cumple que para cualquier $R>0$, los promedios espaciales $F_{R,t}$ poseen una densidad continua y acotada $f_{F_{R,t}}$, y la distancia entre las densidades de los promedios espaciales y la densidad de la distribución normal cumplen que  
\[
...
Siendo este capítulo dedicado a la prueba de este teorema, lo estructuramos de la siguiente forma. Dedicamos primero una sección a mostrar dos resultados clave en la misma. El primero de ellos tiene que ver con una expresión para la segunda derivada de Malliavin de la solución a la ecuación del calor $\eqref{shedefinitiva}$, mientras que el segundo tiene que ver con la existencia de los momentos negativos.

La segunda sección consiste en el cuerpo de la prueba como tal. Justo como se mencionó en la segunda sección del capítulo tres, la cota \eqref{cotafundamental} es la columna vertebral de la prueba, y por lo tanto esta sección comienza verificando que se cumplen las hipótesis de dicho teorema. Posterior a esto, se muestra que de los dos sumandos de la cota \eqref{cotafundamental}, uno de ellos es acotado relativamente fácil por la cantidad buscada, usando para ello parte de la prueba del teorema 1.1 de \cite{HUANG20207170}, mientras que el segundo de ellos, correspondiente a la norma de la segunda derivada de la solución mild $u$, es el que abarca el grueso de la prueba.



\section{Cotas para la primera y segunda derivada de la solución mild y existencia de momentos negativos}

Tal y como se mencionó, estudiamos primero el comportamiento de la segunda derivada de la solución mild $u$ a la ecuación del calor \eqref{shedefinitiva}. Cabe mencionar que para lograrlo, debemos mencionar un resultado análogo para la primera derivada de la solución. Esto debido a que precisamente es a partir de dicho resultado que se llega al que es de nuestro principal interés, además de ser necesario para la prueba del resultado principal. Dicho resultado lo enunciamos a continuación, y se puede consultar en \cite[proposición 2.4.4]{nualart2006malliavin}.

\begin{teo}\label{cotaderivada1} 
 S
 \end{teo}
De acuerdo con \cite{KUZGUN202268}, el siguiente resultado correspondiente a la cota de la segunda derivada de la solución mild resuelve un problema abierto en el cálculo de Malliavin y tiene su interés en sí. Enunciamos dicho resultado a continuación.
 \begin{teo}\label{cotaderivada2}
   ...
   donde $C_{T,p}$ es una constante que depende de $T,p$ y $\sigma$, y 
      ...
      \end{align*} 
   \end{teo}
Tornamos nuestra atención a la existencia de los momentos negativos de la derivada de la solución a la SHE. Este es el contenido del siguiente resultado.
\begin{teo} 
...
\subsection{Validez de las hipótesis}
\subsection{Cota de la segunda derivada de los promedios espaciales}
