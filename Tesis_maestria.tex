...
\begin{dfn}
   Sea $p\geq1$ un entero positivo. Denotamos $$Dom(\delta^p):=\left\{g\in L^2(\gamma) \ : \ \abs{\langle D^{p}f,g\rangle_{L^2(\gamma)}}\leq c\|f\|_{L^2(\gamma)}, \qquad \forall f\in \D^{2,p}\right\},$$ 

   y definimos el operador de divergencia $p$-ésimo como la aplicación $\delta^p:Dom(\delta^{p})\longrightarrow L^2(\gamma)$ tal que asigna a cada $g\in Dom(\delta^{p})$ al único elemento $\delta^{p} g\in L^2(\gamma)$ que, gracias al teorema de representación de Riesz, cumple la igualdad
       \[
       \int_\R D^{p}f(x)g(x) \ d\gamma(x)=\int_\R f(x)\delta^{p}g(x) \ d\gamma(x), \qquad \forall f\in L^2(\gamma).
      \]
\end{dfn}
...
Es de interés encontrar una forma de calcular el operador $\delta$ aplicado a alguna función en $Dom(\delta)$. En este sentido, existe una fórmula para calcular $\delta g$ para $g\in \D^{1,2}$. Este es el contenido del siguiente lema.

\begin{lema}\label{formuladelta}
   Se cumple que $\S\subseteq Dom(\delta)$ y para cualquier $g\in \D^{1,2}$, 
   \[
 ...
 \begin{proof} 
   ...
   \]
   Pero lo anterior ocurre para cualquier $f\in \mathcal{S}$. Esto nos dice que $\mathcal{S}\subseteq Dom(\delta)$ y que para cualquier $g\in \mathcal{S}$, la fórmula anterior es válida. 
 ...
 \begin{proof} 
  ...
  \end{teo}
   \begin{proof} 
     ...
     \[
     P_tf(x)=\E\left[f(X_{t+s}^{x})|\F_s\right]=\E_x\left[f(e^{-(t+s)}x-e^{-(t+s)}B_{e^{2(t+s)}-1})|\F_s\right]=\int_\R f \left(e^{-t}x-\sqrt{1-e^{-2t}}y\right)d\gamma(y),
     \]
...
\subsection{El generador infinitesimal}
 ...
 \]  
Dicho es directamente el dominio del generador infinitesimal $L$. Así, dada $f\in Dom(L)$, se tiene que 
\[
...
En esta sección se presentan los operadores de Malliavin generales, que serán utilizados en lo subsecuente. 
...
\begin{dfn} 
 ...
 \]
 Es de destacar que en la expresión anterior, la distancia entre las leyes de las variables $X$ y $N$ están codificadas en la cantidad de la derecha, la cual no involucra la ley Gaussiana estándar de manera directa.
 
 
 
 
 Un resultado principal en esta sección es el siguiente teorema que nos da una cota explícita universal de las normas de las soluciones a la ecuación de Stein. 
 \begin{teo} 
  Sea $z\in \R$. Entonces la función $f_z$ es tal que 
  \[
      \norm{f_z}_{\infty}\leq \frac{\sqrt{2\pi}}{4} \qquad \text{y} \qquad \norm{f'_z}_\infty\leq 1.
  \]
En particular, para $N$ una variable aleatoria normal estándar, y para cualquier variable integrable $F$, se tiene que 
\[
   d_{\text{Kol}}(F,N)\leq \sup_{f\in \F_{Kol}}\abs{\E\left[f'(F)\right]-\E\left[Ff(F)\right]},
\]
donde $\F_{\text{Kol}}:=\left\{f:\norm{f}_\infty\leq \frac{\sqrt{2\pi}}{4}\right\}$
  \end{teo}
\section{Existencia de densidades}
